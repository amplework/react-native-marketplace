...
const SalesSpecialItem: React.FC<any> = ({ salesSpecial, onPress, onPressShare }) => {  
  return (
      ...
      <CardBody row jc="space-between" ai="center">
        <Box flex ml={12}>
          ...
          <Box row ai='center' >
            ...
            <Paragraph size="xs" type='bold' >{salesSpecial?.service?.name}</Paragraph>
          </Box>
          <Box row>
            <CardSubTitle>{I18n.t('salesSpecial.price')}</CardSubTitle>
            <Paragraph size="s">
              {salesSpecial.salePrice ? `$${salesSpecial.salePrice}` : ''}
            </Paragraph>
          </Box>
        </Box>
